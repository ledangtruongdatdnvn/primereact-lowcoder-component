...
export const defStaticProps = {
  placeholder: 'Choose date(s)', selectionMode: 'range', hideOnRangeSelection: true, dateFormat: 'dd/mm/yy', style: {
    width: '100%',
  },
};
...
let CalendarCompBase = (function () {
  const childrenMap = {
    ...
    value: jsonObjectExposingStateControl('value', {
      start: '02/11/2000', end: '02/12/2000',
    }),
    ...
    onEvent: eventHandlerControl([{
      label: 'onChange', value: 'change', description: 'Triggers when Calendar is changed.',
    },]),
  };
  ...
  return new UICompBuilder(childrenMap, (props: any) => {
    ...
    return (<div style={{padding: '5px'}}>
      <LabelWrapper label={props.label.value} required={props.required.value} error={props.error.value}
        ...
        <Calendar {...props.staticProps} value={input} onChange={handleChange}
                  invalid={props.error.value.length > 0}></Calendar>
      </LabelWrapper>
    </div>);
  })
...
export const exposingConfigs = [new NameConfig('staticProps'), new NameConfig('value'), new NameConfig('label'), new NameConfig('error'), new NameConfig('caption'), new NameConfig('showCaption'), new NameConfig('required'), NameConfigHidden,];
