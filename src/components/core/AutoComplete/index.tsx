...
const defStaticProps = {
  placeholder: 'Enter a country name', field: 'name', inputStyle: {
    width: '100%',
  }, style: {
    width: '100%',
  },
...
const defValue = '';
const defSuggestions = [{name: 'United Kingdom', code: 'UK'}, {name: 'United States', code: 'USA'},];
...
let AutoCompleteCompBase = (function () {
  const childrenMap = {
    ...
    onEvent: eventHandlerControl([{
      label: 'onChange', value: 'change', description: 'Triggers when Auto Complete is changed.',
    }, {
      label: 'completeMethod', value: 'complete', description: 'Triggers when Auto Complete is completed.',
    },]),
  };
...
const exposingConfigs = [new NameConfig('staticProps'), new NameConfig('value'), new NameConfig('suggestions'), new NameConfig('label'), new NameConfig('error'), new NameConfig('caption'), new NameConfig('showCaption'), new NameConfig('required'), NameConfigHidden,];
