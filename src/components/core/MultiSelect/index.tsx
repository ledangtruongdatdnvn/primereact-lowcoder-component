...
const defStaticProps = {
  placeholder: 'Select Cities', optionLabel: 'name', filter: true, style: {
    width: '100%',
  },
};
const defValue = [{name: 'New York', code: 'NY'}];
const defOptions = [{name: 'New York', code: 'NY'}, {name: 'Rome', code: 'RM'}, {
...
let MultiSelectCompBase = (function () {
  const childrenMap = {
    ...
    onEvent: eventHandlerControl([{
      label: 'onChange', value: 'change', description: 'Triggers when MultiSelect is selected.',
    },]),
  };
  ...
  return new UICompBuilder(childrenMap, (props: any) => {
    ...
    return (<div style={{padding: '5px'}}>
      <LabelWrapper label={props.label.value} required={props.required.value} error={props.error.value}
        ...
        <MultiSelect {...props.staticProps} value={props.value.value} options={props.options} onChange={handleChange}
                     invalid={props.error.value.length > 0}></MultiSelect>
      </LabelWrapper>
    </div>);
  })
