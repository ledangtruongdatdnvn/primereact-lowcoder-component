...
const defStaticProps = {
  tooltip: 'Enter a password', inputStyle: {
    width: '100%',
  }, style: {
    width: '100%',
  },
...
let PasswordCompBase = (function () {
  const childrenMap = {
    ...
    onEvent: eventHandlerControl([{
      label: 'onChange', value: 'change', description: 'Triggers when Password is changed.',
    },]),
  };
  ...
  return new UICompBuilder(childrenMap, (props: any) => {
    ...
    return (<div style={{padding: '5px'}}>
      <LabelWrapper label={props.label.value} required={props.required.value} error={props.error.value}
        ...
        <Password {...props.staticProps} value={props.value.value} onChange={handleChange}
                  invalid={props.error.value.length > 0}></Password>
      </LabelWrapper>
    </div>);
  })
