...
const defStaticProps = {
  placeholder: 'Enter a number', min: 1, max: 10, style: {
    width: '100%',
  },
};
const defValue = 1;
  ...
  return new UICompBuilder(childrenMap, (props: any) => {
    ...
    return (<div style={{padding: '5px'}}>
      <LabelWrapper label={props.label.value} required={props.required.value} error={props.error.value}
        ...
        <InputNumber {...props.staticProps} value={props.value.value} onValueChange={handleValueChange}
                     invalid={props.error.value.length > 0}></InputNumber>
      </LabelWrapper>
    </div>);
  })
