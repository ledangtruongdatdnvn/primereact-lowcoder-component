...
const defValue = [{
  id: 'code-1', name: 'row 1', category: 'important', quantity: 1,
}, {
  id: 'code-2', name: 'row 2', category: 'very important', quantity: 2,
}, {
  id: 'code-3', name: 'row 3', category: 'important', quantity: 3,
}, {
  id: 'code-4', name: 'row 4', category: 'less important', quantity: 4,
},];

const defColumns = [{header: 'Column 1', field: 'id'}, {
  header: 'Column 2', field: 'name',
},];
...
let TableCompBase = (function () {
  const childrenMap = {
    ...
    onEvent: eventHandlerControl([{
      label: 'onChange', value: 'change', description: 'Callback to invoke on pagination.',
    },]),
  };
  ...
  return new UICompBuilder(childrenMap, (props: any) => {
    ...
    const bodyTemplate = (rowData) => {
      return (<div className='flex justify-content-center align-items-center gap-2'>
    ...
    return (<DataTable {...props.staticProps} value={props.value} first={props.first.value} rows={props.rows.value}
                       totalRecords={props.totalRecords.value} onPage={(event) => handleChange(event)}>
      ...
      return (<>
        ...
        <Section name='Description'>
          <ol className='text-sm p-0 m-0 mx-3'>
            ...
            <li>'hiddenFieldCondition' and 'hiddenValueCondition' are used for controlling the visibility of the
              action button.
            </li>
            <li>
              'selectedRow' and 'selectedButtonType' are exposed states, used for event checking. Paging buttons will
              have a 'paging' type, while other buttons will have their own predefined types
              in Button Groups.
            </li>
            <li>
...
const exposingConfigs = [new NameConfig('staticProps'), new NameConfig('value'), new NameConfig('columns'), new NameConfig('first'), new NameConfig('rows'), new NameConfig('totalRecords'), new NameConfig('selectedRow'), new NameConfig('selectedButtonType'), NameConfigHidden,];
